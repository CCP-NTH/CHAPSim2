...
def plot_single_monitor_point(point_num, M, skip_start):
    ...
    # Check if we have enough data after skipping initial points
    if len(data) <= skip_start:
        print(f"Warning: Monitor point {point_num} has only {len(data)} data points, cannot skip {skip_start} initial points")
        print(f"Using all available data for point {point_num}")
        skip_start = 0
    
    print(f"Plotting monitor point {point_num}...")
    if skip_start > 0:
        print(f"  - Skipping first {skip_start} data points")
        print(f"  - Using data points {skip_start+1} to {len(data)} (total: {len(data)-skip_start} points)")
    ...
    # Skip initial points and then sample every M points for clearer visualization
    data_trimmed = data[skip_start:]
    time = data_trimmed[::M, 0]
    
    # Plot each variable in its own subplot
    for j, var in enumerate(variables):
        variable_data = data_trimmed[::M, j+1]
...
def plot_individual_monitor_points(N, M, skip_start):
    ...
    if skip_start > 0:
        print(f"Skipping first {skip_start} data points from each file")
    print()
    ...
    for i in range(1, N+1):
        success = plot_single_monitor_point(i, M, skip_start)
        if success:
            successful_plots += 1
...
if __name__ == "__main__":
    ...
    # Get user input for initial data points to skip
    while True:
        try:
            skip_start = int(input("Enter number of initial data points to skip (0 for none, e.g., 100): "))
            if skip_start >= 0:
                break
            else:
    ...
    # Create the plots
    successful = plot_individual_monitor_points(N, M, skip_start)
